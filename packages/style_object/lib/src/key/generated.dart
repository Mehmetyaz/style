...
class StringKey extends StyleKey<String> {
  const StringKey(super.key, super.tag, [this.fixedCount]);

  final int? fixedCount;

  @override
  String read(ByteDataReader byteData) {
  ...
  @override
  int get type => stringType;
}

class JsonKey extends StyleKey<dynamic> {
  const JsonKey(super.key, super.tag, [this.fixedCount]);

  final int? fixedCount;

  @override
  dynamic read(ByteDataReader byteData) {
    var listMeta = readMeta(byteData);
    return json.decode(byteData.getString(listMeta.count));
  }
